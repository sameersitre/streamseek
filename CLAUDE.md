...
- Phase 24: Whole-dashboard hero scoping (`/spotlight`) — `SpotlightParams` gained `movie_genre`/`tv_genre` (Categories genre) **and** `media_scope: 'movie'|'tv'` (Movies/TV filter tabs); `getSpotlight` gates which halves run by `media_scope` when present else genre-presence (movie-only genre omits `tv_genre` → skips the TV half), cache key includes all three. **`curatedDiscoverURL` is now dual-mode:** no genre (default + media_scope) keeps the strict recent+acclaimed `popularity.desc` + 180-day window; genre-scoped (Categories / Documentaries genre 99) switches to `vote_average.desc` + lower vote floors (`SPOTLIGHT_GENRE_MOVIE_VOTES` 150 / `_TV_VOTES` 50) + **no recency window** — fills the hero for low-volume genres like Documentary (was returning 1 result) and keeps the genre hero (acclaim) distinct from the genre Top 10 (popularity, via `discoverByGenre`). All inputs still pass the Phase-10 security sanitizers (`safeId`/`adultFlag`/`safeRegion`). ✅COMPLETE
...
- **Source logos**: `/sources/?apiKey=...` — provider reference data with `logo_100px` URLs; exposed to the app via `POST /api/v2/getSourceLogos` (`{ logos: { [source_id]: url } }`) so the dashboard OTT badge can fall back to a remote logo for any source without a bundled local asset (badge coverage then matches the Details screen). No Watchmode credit after the first server-lifetime call. **Quota-hardened (`getSourceLogos` in `src/apiExternal/apiExternal.ts`):** (1) **durable snapshot** — persisted to `server/data/sourceLogos.snapshot.json` (outside `dist/`, which `prebuild` wipes; override via `SOURCE_LOGOS_SNAPSHOT_PATH`) on first success and reloaded at startup, so an over-quota/dead key never empties the catalogue; commit the snapshot for cross-deploy immunity, or regenerate with `npm run sources:snapshot` (`scripts/fetch-source-logos.ts`, needs a healthy key); (2) **in-flight dedup** — concurrent cold-start callers share ONE Watchmode request (was a race that helped burn the quota); (3) **failure backoff + graceful degrade** — after a failed fetch it backs off 10 min and returns whatever it has (`{}` if nothing) instead of throwing, so `/getSourceLogos` and `fetchOTTPlatforms` keep working (no client 429; the app falls back to bundled logos).
...
### Character images (TVmaze + Fandom + Wikipedia)
- **TVmaze enrichment** (`services/tvmazeCharacters.ts`): TV cast members get `character_image` (in-costume portrait) attached at `getCastDetails` time — TVmaze `/lookup/shows?imdb=…` (axios follows the 301) → `/shows/{id}/cast` → join by **actor name** (character names disagree between TVmaze/TMDB). Free API, no key, CC BY-SA (app shows "Image: TVmaze"). `tvmaze_checked` marker stamped only on definitive answers (incl. not-found) so transient failures retry; a dedicated **image-only backfill branch** upgrades aggregate docs that predate this without a TMDB refetch, fetching `imdb_id` via external_ids when the doc lacks it. TMDB itself has NO character images (`person/images` = headshots; `tagged_images` is community-tagged and near-empty).
- **Fandom page image**: `prop=revisions|pageimages&piprop=original` merged into the existing fandom bio request (zero extra calls) — fandom-sourced bios now carry `thumbnail` like Wikipedia ones.
- **⚠️ Cache-key bug fixed (affects ALL TMDB doc caches)**: stored docs always carried TMDB's **numeric** `id` because the TMDB payload was spread after `req.body` — the app's **string**-id lookups never matched, so `details_cast` / `media` / `details_movie` / `details_tv` were **write-only** (every request re-fetched TMDB + inserted a duplicate). Lookups + stored keys are now canonical `Number(id)` (spread last so payloads can't overwrite), which matches all existing prod docs. Local dedupe removed ~590 duplicates; prod collections may want the same one-time dedupe (group by `{id, media_type}`, keep newest).
