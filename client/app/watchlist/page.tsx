...
import { useUserWatchlist } from "@/app/hooks/queries";
...
function WatchlistContent() {
  ...
  const { data, isLoading } = useUserWatchlist(page);

  if (status !== "authenticated") {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center gap-3">
        <FontAwesomeIcon icon={faBookmark} className="h-12 w-12 text-zinc-600" />
        <p className="text-lg text-zinc-400">Sign in to view your watchlist</p>
  ...
  if (!data?.results || data.results.length === 0) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-zinc-600">
          Browse movies and TV shows to add some!
        </p>
      </div>
    );
  }
...
export default function Watchlist() {
  return (
    <div className="mx-auto max-w-7xl px-6 py-10">
      <h1 className="mb-6 text-3xl font-bold text-white">Watchlist</h1>
